CA.update <- function(Develop, Water, DEM, zones_centers, centers, k, zones, steps, aim_1, aim_2) {
  if (steps<=10) {
    zones_centers <- zones_centers[(zones_centers$DN==1)|(zones_centers$DN==2),]
  }
  if ((10<steps)&(steps<=20)) {
    zones_centers <- zones_centers[(zones_centers$DN==1)|(zones_centers$DN==2)|(zones_centers$DN==3)|(zones_centers$DN==6),]
  }
  if ((20<steps)&(steps<=28)) {
    zones_centers <- zones_centers[(zones_centers$DN==1)|(zones_centers$DN==2)|(zones_centers$DN==3)|(zones_centers$DN==6)|(zones_centers$DN==4)|(zones_centers$DN==24)|(zones_centers$DN==42),]
  }
  value_to_zones <- function(x,zones_centers) {
    values = seq(nrow(zones_centers))*0
    for (i in seq(nrow(zones_centers))) {
      loc <- dist(rbind(x,c(zones_centers$row[i],zones_centers$col[i])))
      values[i] <-zones_centers$z[i]-zones_centers$z[i]*exp(-6+0.6*loc)/(1+exp(-6+0.6*loc))
    }
    return(sum(values))
  }
  ...
  x.dist <- foreach(i = 1:nrow(Water), .combine = rbind, .packages = c('foreach')) %dopar% {
    ...
    for (j in 1:ncol(Water)) {
      if ((!is.na(Develop[i,j]))&(!is.na(Water[i,j]))) {
        if ((Develop[i,j]==0)&(Water[i,j]==1)&(DEM[i,j]==1)) {
          row_result[j] <- value_to_clusters(c(i,j),centers) + value_to_zones(c(i,j),zones_centers)
        }
      }
    }
    row_result
  ...
  if (steps<=10) {
    X = k$k0+k$k1*x.develop+k$k2*x.dist+k$k3*as.numeric(zones==1&zones==2)
  }
  if ((10<steps)&(steps<=20)) {
    X = k$k0+k$k1*x.develop+k$k2*x.dist+k$k3*as.numeric(zones==1|zones==2|zones==3|zones==6)
  }
  if ((20<steps)&(steps<=28)) {
    X = k$k0+k$k1*x.develop+k$k2*x.dist+k$k3*as.numeric(zones==1|zones==2|zones==3|zones==6|zones==4|zones==24|zones==42)
  }
  if (steps>28) {
    X = k$k0+k$k1*x.develop+k$k2*x.dist+k$k3*as.numeric(zones!=7)
  }
...
# import zoning data
zones <- terra::rast('zones.tif')
zones_centers <- terra::vect('zones.shp')
zones_centers <- terra::centroids(zones_centers)
zones_centers <- terra::as.data.frame(zones_centers,geom='XY')
zones_centers$row <- terra::rowFromY(zones,zones_centers$y)
zones_centers$col <- terra::colFromX(zones,zones_centers$x)
zones_centers <- zones_centers[zones_centers$DN!=7,c('DN','row','col')]
zones_centers$z <- 1
zones_centers$z[zones_centers$DN==4] <- 0.8
zones_centers$z[zones_centers$DN==24] <- 0.8
zones_centers$z[zones_centers$DN==42] <- 0.8
zones_centers$z[zones_centers$DN==2] <- 0.6
zones_centers$z[zones_centers$DN==3] <- 0.6
zones_centers$z[zones_centers$DN==6] <- 0.4
zones_centers$z[zones_centers$DN==251] <- 0.2
zones_centers$z[zones_centers$DN==252] <- 0.2
zones_centers$z[zones_centers$DN==351] <- 0.2
zones_centers$z[zones_centers$DN==352] <- 0.2
zones_centers$z[zones_centers$DN==51] <- 0.2
zones_centers$z[zones_centers$DN==52] <- 0.2
zones_centers$z[zones_centers$DN==53] <- 0.2
zones <- terra::as.matrix(zones, wide=TRUE)
zones[is.na(zones)] <- 0
# import GHSL data
# Total
